...
# function "scan_for_reverse_login_attempts" FAILED reverse logins
...
# replays directly after a failed Client auth packet.
  ...
  if (base == 4 && orig == T && len == size_reverse_login_prompt) {
      ...
      NOTICE([$note=SSH_R_Reverse,
          $msg = fmt("Reverse Shell failed logon from %s to %s",c$id$orig_h,c$id$resp_h),
          $sub = fmt("Reverse Shell failed logon")]); 
